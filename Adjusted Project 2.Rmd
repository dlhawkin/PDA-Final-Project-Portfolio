...
After exploring the categorical relationship, we will observe the Pearson correlations of all the variables. Based on this plot, we we will observe potential risk factors to explore in our logistic model.
...
## Logistic
The logistic regression model doesn't use a penalty term and it's objective is to model the probability of a binary outcome Pr(Y=1|X) OR Pr(Y=0|X). Logistic regression uses the MLE (Maximum Likelihood Estimate) of the observed data without imposing any regularization on the coefficients. Logistic regression finds the coefficients that best predict the outcome based on the given predictors by maximizing the likelihood (or minimizing the negative log-likelihood).After filtering the statistical significant variables using $\alpha<=.05$, we will observe the variables that are left after pooling the results.
...
hedonsum_y_pq_plot<-ggplot(project2_table, aes(x = hedonsum_y_pq1 , fill = abst)) +
  ...
  theme_minimal() +
  labs(title = "Baseline Characteristics of Abstinence Status and Treatment Group using Complementary Reinforcers ",
...
nmr_plot<-ggplot(project2_table, aes(x = NMR , fill = abst)) +
  ...
  facet_wrap(~treatment_groups, scales = "free")+
  theme_classic()+
  labs(title = "Baseline Characteristics of Abstinence Status and Treatment Group using NMR",
       x = "NMR",
       y = "Density")
...
hedonsum_N_plot<-ggplot(project2_table, aes(x = hedonsum_n_pq1 , fill = abst)) +
  ...
  facet_wrap(~treatment_groups, scales = "free")+
  theme_classic()+
  labs(title = "Baseline Characteristics of Abstinence Status and Treatment Group using Substitute Reinforcers",
       x = "Hedonsum",
       y = "Density")
scale_fill_manual(values = c("1" = "pink", "0" = "blue"), name = "Abstinence Status")
...
age_plot<-ggplot(project2_table, aes(x =age_ps , fill = abst)) +
  ...
  facet_wrap(~treatment_groups, scales = "free")+
  theme_classic()+
  labs(title = "Baseline Characteristics Hedonsum by Treatment Group s(Continuous 1)",
       x = "Age",
       y = "Density")
scale_fill_manual(values = c("1" = "pink", "0" = "blue"), name = "Abstinence Status")
...
anhedonia_plot<-ggplot(project2_table, aes(x =shaps_score_pq1 , fill = abst)) +
  ...
  facet_wrap(~treatment_groups, scales = "free")+
  theme_classic()+
  labs(title = "Baseline Characteristics Anhedonia by Treatment Groups",
       x = "Anhedonia",
       y = "Density")
scale_fill_manual(values = c("1" = "pink", "0" = "blue"), name = "Abstinence Status")
...
income_plot<-ggplot(project2_table, aes(x = abst, fill = inc)) +
...
# Create a stacked bar plot
education_plot<-ggplot(project2_table, aes(x = abst, fill = edu)) +
  geom_bar(position = "fill") + # Use position = "fill" for proportions
  labs(
    x = "Abstinence",
    y = "Proportions",
...
library(patchwork)
combined_plot <- (Black_plot | NHW_plot) / Hisp_plot
combined_plot


grid.arrange(
  Black_plot, NHW_plot, Hisp_plot, 
  ncol = 2,  
  nrow = 2,
...
#project2_table$edu <- factor(project2_table$edu)
...
```{r}
#project2_table$ftcd_score <- as.integer(project2_table$ftcd_score)
#project2_table$edu <- factor(project2_table$edu)
set.seed(222)
...
for (i in 1:5) {
  ...
  test_data1$predicted_probs_main_test <- predict(main_effects, newdata = test_data1, type = "response")
  ...
  test_data1$predicted_probs_interaction_test <- predict(interaction_effects, newdata = test_data1, type = "response")
