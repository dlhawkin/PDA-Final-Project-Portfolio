...
In this analysis, we will utilize line plots to examine trends in runtimes (performance) across various ages, highlighting the relationship between male and female marathon performances. Additionally, we will analyze summary statistics of runtimes across different age ranges to identify the age groups with the fastest (highest-performing) runtimes. Linear regression models will be employed to investigate the relationship between runtimes and other key variables, providing insights into potential predictors of performance. Furthermore, correlation plots will be used to assess the strength and direction of relationships among variables, offering a comprehensive understanding of their interdependencies.
...
In the **Marathon Performance Summary Table by Age Ranges** and the **Effects of Age on Marathon Performance in Men and Women Boxplot**, we noticed some summary statistics of the boxplot measurements of the marathon runners` runtimes based upon their age ranges. The marathon runners were broken up into seven age groups where we noticed a somewhat balance number of people in each group. From the box plot, younger runners <15–25 tend to perform better, with lower runtimes, as evidenced by lower median values and a tighter interquartile range (IQR). This age group shows more variability (outliers) in performance for both genders.In the summary table, the mean runtime for this group is 184 minutes, with a median of 177 minutes. This group performs significantly better than older age groups, with a relatively smaller IQR of 49.
...
The table shows a continued decline in median runtimes as runners transition from 26–35 (median: 148 minutes) to 36–45 (median: 163 minutes). The IQR for both groups remains relatively small (25), indicating more consistent performance in this age range.Men tend to outperform women in these age groups, as seen by the lower median runtimes in the blue box plots.

The oldest age groups 66–75 and 76+ exhibit the largest runtimes and widest performance variability. The box plots show much larger IQRs, particularly for women, reflecting significant variation in performance.In the table, the mean and median runtimes increase for the 66–75 (mean: 269; median: 259) and 76+ (mean: 351; median: 345) groups. The IQR also expands especially for the 76+ group (IQR: 95), indicating a broad range of abilities within this age range.The box plot demonstrates that men, even in older age groups, continue to have better performance (lower runtimes) than women. However, the performance gap between genders widens significantly after age 66, particularly in the 76+ group.
...
## Question Aim 1: Examine effects of increasing age on marathon performance in men and women.
...
To begin this analysis, a linear model will be constructed to demonstrate the relationship and impact of environmental conditions on marathon performance, considering differences across age and gender. The outcome variable is the marathon runners' runtimes in minutes, analyzed in conjunction with environmental conditions, gender, and age. Upon examining the model, it was noticed that the weather parameters highlighted in blue, such as `Dry Bulb Temp C` (p=0.00008), `Percent Relative Humidity` (p=0.00000), `Black Globe Temp C` (p=0.000086), `Dew Point in C` (p=0.00955), `Solar Radiation` (p=0.00000), and `Wet Bulb Globe Temp` (p=0.00031), demonstrated statistical significance (p<0.05). Other significant parameters include the interaction terms, such as `Age (yr)` (p=0.00000), `GenderM` (p=0.00001), `Dry Bulb Temp C:Age (yr)` (p=0.00001), `Percent Relative Humidity:Age (yr)` (p=0.0000), `Black Globe Temp C:Age (yr)` (p=0.00029), `Percent Relative Humidity:Gender` (p=0.02116), `Age (yr):GenderM` (p=0.01190), `Age (yr):Wet Bulb Globe Temp` (p=0.00017), `Solar Radiation:Age (yr)` (p=0.00000), and `Dew Point in C:Age (yr)` (p=0.0073). *These significant weather parameters were used in further analysis in the Supplementary Materials below.*

```{r}
 # Create Dataframe to allocate for other environmental conditions
 environmental_conditions <- course_record_project1 %>%
  dplyr::select(Race, Gender, `Age (yr)`, Runtimes_Minutes, `Dry bulb Temp C`, `Wet bulb Temp C`,
...
```{r}
plot(
  environmental_conditions$`Age (yr)`, 
  environmental_conditions$Runtimes_Minutes,
  main = "Relationship Between Age and Runtimes",
  xlab = "Age ",
  ylab = "Runtimes (Minutes)",
  col = "blue",
  pch = 16
)
...
plot(
  environmental_conditions$`Age (yr)^2`,
  environmental_conditions$Runtimes_Minutes,
  main = "Relationship Between Age Squared and Runtimes",
  xlab = "Age Squared (Age^2)",
  ylab = "Runtimes (Minutes)",
  col = "blue",
  pch = 16
)
...
spline_model <- lm(Runtimes_Minutes ~ 
                     ...
                     `Black Globe Temp C`:Gender + 
                     `Wind Speed`:Gender + 
                     `Dew Point in C`:Gender + 
...
spline_table %>%
  kbl(caption = "Spline Modle:Impact of Environmental Conditions on Marathon Performance by Age and Gender",
 ...
 row_spec(11, bold = TRUE, color="black", background = "#87CEEB")%>%
  ...
  row_spec(20, bold = TRUE, color="black", background = "#87CEEB")%>%
   row_spec(21, bold = TRUE, color="black", background = "#87CEEB")%>%
   row_spec(24, bold = TRUE, color="black", background = "#87CEEB")
...
inv2lims<- range(environmental_conditions$`Age (yr)`)
inv2.grid<- seq(from= inv2lims[1], to= inv2lims[2])
pred <- predict(spline_model, newdata = environmental_conditions= inv2.grid), se = TRUE)

# Find the index of the minimum predicted value
min_index <- which.min(pred$fit)

# Get the corresponding age and predicted value
min_age <- inv2.grid[min_index]
min_fit <- pred$fit[min_index]

# Print the results
cat("The minimum age is:", min_age, "with a predicted runtime of:", min_fit, "\n")




length(inv2.grid) # Check length of the grid
length(pred$fit)  # Check length of predictions
pred

# Corrected segments function
plot(environmental_conditions$`Age (yr)`, environmental_conditions$Runtimes_Minutes,
     main = "Regression Spline Plot", xlab = "Age", ylab = "Runtime_Minutes")

# Add spline fit line
lines(inv2.grid, pred$fit, col = 'green', lwd = 3)

# Add confidence intervals (upper and lower bounds)
lines(inv2.grid, pred$fit + 4 * pred$se.fit, lty = "dashed", lwd = 4, col = 'red')  # Upper CI
lines(inv2.grid, pred$fit - 4 * pred$se.fit, lty = "dashed", lwd = 4, col = 'red')  # Lower CI

# Correct segments to add vertical lines at knots
segments(x0 = 20, y0 = 0, x1 = 20, y1 = max(environmental_conditions$Runtimes_Minutes), col = "black")
segments(x0 = 40, y0 = 0, x1 = 40, y1 = max(environmental_conditions$Runtimes_Minutes), col = "black")
segments(x0 = 60, y0 = 0, x1 = 60, y1 = max(environmental_conditions$Runtimes_Minutes), col = "black")
segments(x0 = 80, y0 = 0, x1 = 80, y1 = max(environmental_conditions$Runtimes_Minutes), col = "black")
...
linear_model <- lm(Runtimes_Minutes ~ `Age (yr)`, data = environmental_conditions)
summary(linear_model)

plot(environmental_conditions$`Age (yr)`,environmental_conditions$Runtimes_Minutes,main = "Comparison of Linear and Spline Regression",
     xlab = "Age (yr", ylab = "Runtimes Minutes(performance)")
lines(inv2.grid, predict(linear_model, newdata = list(`Age (yr)` = inv2.grid)), col = 'blue')
lines(inv2.grid, pred$fit, col = 'red')
legend("topleft", legend = c("Linear Regression", "Spline Regression"), 
...
names(environmental_conditions)
...
# Define the knots
knots <- c(25, 35, 45, 55,65,75 )

# Fit a linear model with spline regression
spline_model2 <- lm(Runtimes_Minutes ~ ns(`Age (yr)`, knots = knots) ,
                   data = environmental_conditions)
...
inv2lims1<- range(environmental_conditions$`Age (yr)`)
inv2.grid1<- seq(from= inv2lims1[1], to= inv2lims1[2])
pred1 <- predict(spline_model2, newdata = list(`Age (yr)` = inv2.grid1), se = TRUE)
...
# Add spline fit line
lines(inv2.grid1, pred1$fit, col = 'green', lwd = 3)

# Add confidence intervals (upper and lower bounds)
lines(inv2.grid1, pred1$fit + 4 * pred1$se.fit, lty = "dashed", lwd = 4, col = 'red')  # Upper CI
lines(inv2.grid1, pred1$fit - 4 * pred1$se.fit, lty = "dashed", lwd = 4, col = 'red')  # Lower CI

# Correct segments to add vertical lines at knots
segments(x0 = 25, y0 = 0, x1 = 25, y1 = max(environmental_conditions$Runtimes_Minutes), col = "black")
segments(x0 = 35, y0 = 0, x1 = 35, y1 = max(environmental_conditions$Runtimes_Minutes), col = "black")
segments(x0 = 45, y0 = 0, x1 = 45, y1 = max(environmental_conditions$Runtimes_Minutes), col = "black")
segments(x0 = 55, y0 = 0, x1 = 55, y1 = max(environmental_conditions$Runtimes_Minutes), col = "black")
segments(x0 = 65, y0 = 0, x1 = 65, y1 = max(environmental_conditions$Runtimes_Minutes), col = "black")
segments(x0 = 75, y0 = 0, x1 = 75, y1 = max(environmental_conditions$Runtimes_Minutes), col = "black")
...
plot(environmental_conditions$`Age (yr)`,environmental_conditions$Runtimes_Minutes,main = "Comparison of Linear and Spline Regression",
     xlab = "Age (yr", ylab = "Runtimes Minutes(performance)")
lines(inv2.grid1, predict(linear_model, newdata = list(`Age (yr)` = inv2.grid1)), col = 'blue')
lines(inv2.grid1, pred1$fit, col = 'red')
legend("topleft", legend = c("Linear Regression", "Spline Regression"), 
       col = c("blue", "red"), lty = 1)
...
Above, we noticed that age has a non-linear relationship and has like a U-shape. By this observation, we examined the squared version of age to make it linear, by it still has that U-shape. Therefore, we will examine a spline regression versus a linear regression to see if we a more linear trend. The spline regression model is implemented here to model non-linear relationship between ages and runtimes by dividing In order to observe the importance of age impact in this linear regression model, we will exclude the age variable and observe the AIC and BIC to see witness lowest AIC and BIC  metric which will inform us about the best model that will inform our decision.


In this model, we analyzed the spline regression  to examine the relationship and impact of environmental conditions on marathon performance, considering gender differences while excluding age. The outcome variable, marathon runners' runtimes, was assessed in relation to environmental conditions and gender. The results indicate that race parameters such as `RaceC` (p < 0.00001), `RaceD` (p < 0.00001), `RaceNY` (p < 0.00001), and `RaceTC` (p < 0.00001) are statistically significant predictors of performance. However, weather parameters were not found to be significant, suggesting that gender differences do not influence how runners are affected by varying weather conditions. Similarly, interaction terms between gender and weather parameters failed to show significance, indicating that the impact of weather does not differ meaningfully across genders. Overall, these findings highlight the critical role of race location and gender in shaping marathon performance, while environmental factors appear to be less influential.
...
colnames(vif_values) <- c("GVIF", "DF","Adjusted GVIF") 


high_vif_variables<-vif_values%>%
  filter(GVIF>=10)

# Extract variables with VIF > 10
high_vif <- vif_values[vif_values > 10]


 high_vif_variables %>%
  kbl(caption = "High VIF Variables",
      booktabs = TRUE, escape = FALSE, align = "c") %>%
  kable_styling(full_width = FALSE, latex_options = c('hold_position'))%>%
  column_spec(1, bold = TRUE, color="black", border_right = TRUE)
 ...
 vif_values3<- vif(spline_model2)
vif_values3<-as.data.frame(vif_values3)
colnames(vif_values3) <- c("GVIF", "DF","Adjusted GVIF") 


high_vif_variables3<-vif_values3%>%
  filter(GVIF>=10)
...
high_vif <- vif_values[vif_values > 10]
 ...
 #vif_values2<- vif(correct_model)
#vif_values2<-as.data.frame(vif_values2)
#colnames(vif_values2) <- c("GVIF", "DF","Adjusted GVIF") 


#high_vif_variables2<-vif_values2%>%
 # filter(GVIF>=10)

# Extract variables with VIF > 10
#high_vif2 <- vif_values2[vif_values2 > 10]
...
The correlation heatmap reveals key insights into the relationship between environmental conditions and marathon performance. The strongest positive correlation is observed between "Age (yr)" and "Runtimes_Minutes" (0.67), indicating that older individuals tend to have slower runtimes, likely due to age-related physiological changes affecting performance. Environmental factors such as "Percent Relative Humidity," "Solar Radiation," and "Dew Point in C" show weak correlations with runtimes, suggesting minimal direct impact on performance in this dataset. However, a strong positive correlation (0.9) between "Wet Bulb Globe Temp" and "Dew Point in C" highlights potential multicollinearity concerns, as these measures likely capture overlapping aspects of heat and humidity. Additionally, a moderate negative correlation (-0.48) between "Percent Relative Humidity" and "Solar Radiation" reflects typical weather patterns, where higher humidity often corresponds to reduced solar exposure.The correlation of (0.67) between `Age and Runtimes` is higher than most correlations with environmental conditions, suggesting that age has a stronger impact on performance than many environmental factors. This indicates that age plays a significant role in marathon performance, regardless of environmental conditions.
...
numeric_data <- course_record_project1%>%
  dplyr::select(Runtimes_Minutes, `Percent Relative Humidity`,
...
According to my Generalized Additive Model (GAM) is an extension of linear models that allows for flexible modeling of non-linear relationships between predictors and a response variable. The flag conditions that has a large impact on marathon performers are Red abd Yellow conditions especially on males.

```{r}
library(mgcv)
gam_model <- gam(Runtimes_Minutes ~ Flag+ Gender, data =weather_parameters)
summary(gam_model)


plot(weather_parameters$Flag,weather_parameters$Runtimes_Minutes)

aov_model <- aov(Runtimes_Minutes ~ Flag, data = weather_parameters)
summary(aov_model)
...
## Answer Aim 3: Identify the weather parameters (WBGT, Flag conditions, temperature, etc) that have the largest impact on marathon performance.
...
The exploratory data analysis demonstrates considerable impact of environmental conditions on marathon performance across both genders and age groups.  According to *Aerobic Performance is Degraded Despite Modest Hypothermia in Hot Environments*, "Endurance exercise performance is degraded within increasing environmental temperature, and the decline in performance is associated with warmer temperature is magnified with longer-distance events such as the marathon foot race (Ely, B. R., et.al). Studies have shown that marathon runners prefer cooler temperatures rather than warmer and humid temperatures due to the body regulating excessive amounts of heat. Running in cooler weather conditions, can reduce risks of heat exhaustion or heat strokes which are common in warmer conditions. When the temperature is lower, the cardiovascular system doesn't have to work as hard to cool the body.This help runners maintain pace without their heart rate increasing as much in hotter conditions.  Runners typically achieve better performance under Green flag conditions, which denote safer weather scenarios with lower Wet Bulb Globe Temperature (WBGT), evidenced by the lower median runtimes in comparison to more severe conditions marked by Red, White, and Yellow flags. With the transition to Red and Yellow flags, indicative of higher WBGT levels, there is a distinct drop in performance, seen in the increased median runtimes. This performance decline under harsher weather conditions can be linked to elevated temperatures and humidity, which are characteristic of these flags. Furthermore, the increased variability in performance under Red and Yellow flags may suggest a broad spectrum of individual heat tolerances among runners. Although both male and female runners exhibit similar patterns in response to changing flag conditions, females consistently display slightly higher median runtimes in all conditions indicating slower runtimes.



# Limtations

The limitations I want to highlight in this study include the non-linear relationship between age and runtimes. To address this limitation, I implemented a spline regression model, as it is better suited for handling non-linear relationships. Additionally, there was a significant indication of high variance inflation factors (VIF) among the environmental conditions, which led to issues of high correlations and multicollinearity. To resolve this problem, environmental conditions such as `Black Globe Temp C`, `Dry Bulb Temp C`, and `Wet Bulb Temp C` were removed to reduce the high correlations.
