...
```{r}
# Initialize a list to track variable selection counts across imputations
variable_selection_counts <- data.frame(Variable = rownames(lasso_coef_estimates[[1]]), 
                                        Selection_Count = 0)

# Loop through each imputation's coefficient estimates
for (i in 1:m) {
  non_zero_vars <- rownames(lasso_coef_estimates[[i]])[lasso_coef_estimates[[i]][,1] != 0]
  variable_selection_counts$Selection_Count[variable_selection_counts$Variable %in% non_zero_vars] <- 
    variable_selection_counts$Selection_Count[variable_selection_counts$Variable %in% non_zero_vars] + 1
}

# Filter for variables selected in more than half of the imputations
selected_vars_consistent <- variable_selection_counts[variable_selection_counts$Selection_Count > m/2, ]

# Merge with the pooled results for mean and standard error
best_subset <- merge(selected_vars_consistent, lasso_pooled_results, by = "Variable")

# Sort by absolute value of the mean coefficient (or by selection count if preferred)
best_subset_sorted <- best_subset[order(-abs(best_subset$Mean), -best_subset$Selection_Count), ]

# Create a table using kable to display the best subset of variables
kable_best_subset <- kable(best_subset_sorted, caption = "Best Subset of Variables Selected by Lasso Model") %>%
  kable_styling(full_width = F, font_size = 12)
